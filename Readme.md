<h1 align="center">Final Fantasy XII: Revenant Job System (PS2 Ver.) </h1>

<p align="center">
  <img src="images/RJS%203.5%20Banner%20New.png" alt="Final Fantasy XII: Revenant Job System v3.5">
</p>

<p align="center">
  <b>A complete gameplay overhaul for Final Fantasy XII International Zodiac Job System (PS2).</b><br>
  Rebuilt from the ground up with brand-new mechanics, rebalanced progression, restored content, and a modern gameplay philosophy inspired by classic Final Fantasy titles.
</p>

---

# Introduction

What started as a simple dream eventually became years of dedication.

I always wanted to create something that could stand beside the greatest Final Fantasy XII mods ever made. That ambition led me to learn English, study game modding, understand the game's internal systems, and ultimately choose the PlayStation 2 version as my platform.

While most projects focus on the PC release, I wanted to push the original hardware beyond what seemed possible.

The result is **Revenant Job System**—a complete gameplay overhaul that redesigns nearly every major gameplay system while preserving the identity that made Final Fantasy XII unforgettable.

Every feature has been handcrafted, tested, balanced, and refined to deliver what I believe is the definitive way to experience **Final Fantasy XII International Zodiac Job System**.

---

# Main Features

## Completely Overhauled Equipment

<p align="center">
    <img src="images/Vaan%20Banner%20Equipment.png" width="100%">
</p>

The entire equipment progression has been redesigned from beginning to end.

Legendary relics, restored beta content, iconic weapons, and entirely new equipment choices transform progression into a rewarding journey where every new weapon truly matters.

---

## Enemy Overhaul

<p align="center">
    <img src="images/Basch%20Banner%20Enemy.png" width="100%">
</p>

A stronger arsenal deserves stronger opponents.

Every regular enemy has been rebalanced with higher overall stats and improved progression to create a more engaging experience reminiscent of classic Final Fantasy titles.

Many enemies have also received new Steal rewards, encouraging exploration and rewarding preparation.

---

## Harder Bosses

<p align="center">
    <img src="images/Gabranth%20Banner%20Harder%20Bosses.png" width="100%">
</p>

Boss encounters have been completely redesigned.

Higher HP, MP, improved endurance, and more demanding battles ensure that every major encounter feels memorable and worthy of its place in the story.

Victory now requires preparation, strategy, and mastery of the game's systems.

---

## Magicks & Technicks Overhaul

<p align="center">
    <img src="images/Ashe%20Banner%20Magicks%20and%20Techs.png" width="100%">
</p>

Every elemental spell and Technick has been rebuilt from scratch.

New spell tiers, restored elements, redesigned progression, and entirely new abilities greatly expand combat possibilities while preserving the balance of the game.

From Water, Aero and Earth magic to powerful Technicks like **All-Out**, combat now offers significantly more tactical depth.

## Vital Stealing

<p align="center">
  <img src="images/Vayne%20Banner%20Vital%20Stealing.png" width="100%">
</p>

Stealing has become an essential gameplay mechanic. Many high-tier weapons, crafting materials, and exclusive rewards can only be obtained from special enemies and bosses, making every successful steal a meaningful part of your progression.

---

## New Exclusive Skills & Spells

<p align="center">
  <img src="images/Reks%20Banner%20Exclusive%20Skills.png" width="100%">
</p>

Each playable character now begins the game with unique Innate Skills independent of their selected Job, giving every party member a stronger identity and new strategic possibilities throughout the adventure.

---

## The 12 Job Classes Renewed

<p align="center">
  <img src="images/Balthier%20The%2012%20Jobs.png" width="100%">
</p>

The License Board has been carefully redesigned to provide greater freedom without sacrificing each Job's individuality. Espers can now be assigned more flexibly while preserving exclusive advantages unique to every class.

---

## Colored Elementals

<p align="center">
  <img src="images/Penelo%20Banner%20Colored%20Element.png" width="100%">
</p>

Elemental weapons, Magicks, ammunition, and related equipment have been color-coded, making elemental affinities immediately recognizable during exploration and combat.

---

## New Bazaar Items

<p align="center">
  <img src="images/Vossler%20Banner%20Baazar.png" width="100%">
</p>

The Bazaar has been significantly expanded with brand-new recipes, conversions, and exclusive rewards. Discover powerful equipment by combining rare loot and making full use of the new crafting possibilities.

---

## Accessories Overhaul

<p align="center">
  <img src="images/Reddas%20Banner%20Accessories.png" width="100%">
</p>

Every accessory in the game has been completely reworked with new attributes and unique effects, transforming them into meaningful equipment choices capable of defining entire character builds.

---

## New Balanced Shops

<p align="center">
  <img src="images/Larsa%20Banner%20Balanced%20Shops.png" width="100%">
</p>

Every shop has been rebuilt specifically for Revenant Job System, providing a smoother progression curve and ensuring that equipment availability matches the redesigned gameplay experience.

---

## Overhauled Ammunition!

<p align="center">
  <img src="images/Fran%20Banner%20Ammunition.png" width="100%">
</p>

Archery, firearms, and bombs have received entirely new ammunition types and effects, including Holy Arrows, Holy Shot, DARK Chaos Bombs, and many other additions that greatly expand ranged combat.

---

# Installation
- Own a Original IZJS ISO from your Backups, DVD or Whatever
- Rename your FFXII IZJS English ISO to FFXII.ISO
- click on Install.bat
- Wait, do not Panick
- Done, play and done

---

- Soon after instalation open your Modified PS2 or PCSX2 with FFXIIRJS.ISO
- You will notice the Silenced Music from Menu and the New Logo

---

# Coming Soon

More screenshots, gameplay previews, documentation, and additional feature showcases.

---

# Ownership & Permissions

This mod was developed entirely from scratch.

No gameplay content was copied from other mods.

Every gameplay system, rebalance, and implementation was created specifically for Revenant Job System.

---

# Credits

- **Sky Pirate's Den** — Community support and valuable feedback.
- **ffgriever** — ISO patcher.
- **Xeavin** — Modding research, reserve license patch, and tools.
- **Aquarius Camus** — Modding guidance and development tools.
- **Akita** & **Shrimpman** — Beta testing.
- **FehDead** — Ideas and project support.
- **Paulo Ítalo** & **João** — Weapon restoration ideas.
- **MidnightZR** & **KellueHaze** — Extensive testing and project support.
- **Mikachuuu** — Gemini Plus assistance and brainstorming.
- **Leandro Kuogetsu** — ChatGPT Plus and Help.
- **Yao** - Full Hand Assistance and Help to help me to Learn how to make Graphics.
